...
| `volumePermissions.enabled`                   | Enable init container that changes volume permissions in the registry (for cases where the default k8s `runAsUser` and `fsUser` values do not work), this will only be run when `master.persistence.enabled` and `securityContext.enabled` are enabled too | `false`                                                 |
